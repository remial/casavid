...
interface ClientStoryIntroProps {
  session: any; // Accept session as a prop
}

const ClientStoryIntro: React.FC<ClientStoryIntroProps> = ({ session }) => {
  const [isSubscribed, setIsSubscribed] = useState(false); // State for subscription status
  const [loading, setLoading] = useState(true); // State for loading

  const videos = [
    { id: 1, url: "https://vidnarrate.sfo3.cdn.digitaloceanspaces.com/Vidnarrate%20Intro.mp4" },
  ...
  return (
    <div className="min-h-screen grainy p-4 flex flex-col items-center">
      <h1 className="text-2xl font-bold mb-8 text-center text-blue-600"></h1>
      {loading ? (
        <p>Loading...</p> // Display a loading indicator while checking subscription
      ) : (
        <div className="relative max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-1 gap-6 items-center">
...
export default ClientStoryIntro;
