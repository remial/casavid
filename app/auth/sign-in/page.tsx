...
const gtag_report_conversion = (url?: string) => {
  if (window.gtag) { // Check if gtag is defined
    window.gtag('event', 'conversion', {
      'send_to': 'AW-11323828600/x8C9CPObh58ZEPjSz5cq',
      'event_callback': () => {
        if (url) {
...
const SignIn: React.FC = () => {
  ...
  const handleCredentialsSignIn = async (event: React.FormEvent) => {
    ...
    if (result?.ok) {
      ...
      window.location.href = '/dashboard'; //this is to force a page refresh
    } else {
  ...
  const handleOAuthSignIn = (providerId: string) => {
    signIn(providerId, { callbackUrl: `${window.location.origin}/dashboard` })
      .then((result) => {
        if (result?.ok) {
          // Assuming this is the success path for Google sign-in
          // Call the conversion tracking function here
          gtag_report_conversion(`${window.location.origin}/dashboard`);
        }
      })
      .catch((error) => {
  ...
  return (
    <div className="py-20 px-5">
      ...
      <div className="logo flex justify-center items-center mb-6 rounded-full">
        <p className="text-bold text-blue-800 font-bold">Sign in to continue </p>
