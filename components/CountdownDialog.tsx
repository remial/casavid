...
const CountdownDialog: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const countdownStart = 17;
  const [countdown, setCountdown] = useState(countdownStart);
  const [progressSections, setProgressSections] = useState(0); // Number of filled sections
  const totalSections = 10; // Total sections in the progress bar
  ...
  useEffect(() => {
    ...
    const updateProgress = (sectionsToAdd: any, delay: any) => {
      setTimeout(() => {
        setProgressSections((prevSections) => Math.min(prevSections + sectionsToAdd, totalSections-1));
      }, delay);
    };
  ...
  return (
    <Dialog open={countdown > 0}>
      <DialogContent className="bg-white">
        <DialogTitle className="text-center text-green-600">Good choice! When the story appears, Enjoy, then Scroll down and pick one of Options 1, 2, 3 to continue...</DialogTitle>
        {/* Progress bar */}
